(function ( $ ) {
    $.fn.drawPage = function( options ) {
       ...
       return this.each(function(){
           ...
           $this.empty();
           var $preTag = '<li class="page-item page-item-li"><a class="page-link paging-real-link" href="javascript:void(0)" tabindex="-1"';
           var $endTag = '</a></li>';
           $this.append($preTag + 'id="page-prev"  data-val="prev"><img src="./assets/img/left.png"  data-index="0" data-val="prev">' + $endTag);
           var $index = 0;
           for (var i = settings.from; i <= settings.to; i++) {
               $this.append($preTag + 'data-index="'+(++$index)+'">'+ i + $endTag);
           }
           $this.append($preTag + 'id="page-next" data-index="'+(++$index)+'" data-val="next"><img src="./assets/img/right.png" data-val="next">' + $endTag);
       });
    }
       ...
       return this.each(function(){
           ...
           $(this).children('ul').click(
               function(event) {
                   ...
                   var $drawMain = false;
                  if($(event.target).attr('data-val') != ''){
                   switch ($(event.target).attr('data-val')){
                    case 'prev':
                        if ($page > 1) {
                            $page = $page - 1;
                            $drawMain = true;
